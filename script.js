...
setInterval(() => {
  const time = new Date();
  var exactTime = time.toLocaleString("en-Us", {
    hour12: true,
    timeStyle: "medium",
    localeMatcher: "lookup",
  });
  var exactDate = time.toLocaleString("en-Us", {
    hour12: true,
    weekday: "short",
    day: "2-digit",
    month: "short",
...
function getCurrentLocationData() {
  navigator.geolocation.getCurrentPosition((success) => {
    ...
    fetch(
      ...
      .then((name) => {
        fetch(
          `https://api.openweathermap.org/data/2.5/onecall?lat=${latitude}&lon=${longitude}&exclude=hourly,minutely,alerts&appid=${API_KEY}&units=metric&lang=en`
        )
          .then((response) => response.json())
          .then((data) => {
...
function convert(data) {
  ...
  var time = dateObject.toLocaleString("en-US", {
    hour12: true,
    timeStyle: "short",
  });
  return time;
